...
3. **Efecto propio — Transición de rutas** (`App.vue`): Fade + slide de 300ms entre rutas usando `<Transition>` de Vue con `mode="out-in"`.
...
- **Scroll-to-section** (Mejora #3): Los links del NavBar navegan suavemente a la sección correspondiente.
...
│   ├─ views/
│   │   ├─ HomeView.vue         ← Landing principal
│   │   └─ CvView.vue           ← Vista de CV imprimible
...
- **Axis (Robot Azul)**: Personaje decorativo de la familia tecnológica de la marca.
...
1. **Navegación**: Los links del navbar deben hacer scroll suave a las secciones
2. **Responsive**: Verificar en mobile (cards 1 col), tablet (2 cols), desktop (3 cols)
3. **Animaciones**: Observar el stagger de las cards al cargar cada sección
4. **Accesibilidad**: Navegar con Tab, verificar focus visible en todos los elementos
